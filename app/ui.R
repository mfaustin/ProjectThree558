      ...
      tabsetPanel(
        ...
        tabPanel("Model Info",
          fluidPage(
            ...
            h5("The general muliple linear regression model:"),
...
R_{2}(j, s)=\\left\\{x \\mid x_{j} \\geq s\\right\\}$$'),
            h5("Using mean reponse in each region, the goal is to find s and j values to minimize the following equation:"),
...
$$'),
            ...
            h5("The main advantage of random forest method is that by averaging results across many trees it decreases variance comparted to a single tree fit.  Random forest improves over bagging by using the random subset of predictors which reduces variance compared to bagging and usually results in a better fitting model."),
            br(),br(),br()
          )         
        ),
        ##Model Fitting Tab
